...
//     k_buffer = kmalloc_array(max_entries, sizeof(memory_limitation_t), GFP_KERNEL);
//     if (!k_buffer) {
//         return -ENOMEM;
//     }
...
//         // Copiar datos válidos al buffer
//         k_buffer[count].pid = temp->pid;
//         k_buffer[count].memory_limit = temp->memory_limit;

//         pr_info("taro_get_memory_limits: Iterando PID=%d, Limit=%zu\n", temp->pid, temp->memory_limit);
//         count++;
//     }

//     mutex_unlock(&memory_limit_mutex);

//     if (copy_to_user(u_processes_buffer, k_buffer, count * sizeof(memory_limitation_t))) {
//         kfree(k_buffer);
//         return -EFAULT;
//     }
